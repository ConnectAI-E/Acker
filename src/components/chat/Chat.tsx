...
const Chat: React.FC = function Chat() {
  ...
  const renderRetryButton = useMemo(() => ((loading || (data.length > 0 && !assistantLoading)) ? (
    <button type="button" className="btn flex justify-center gap-2 btn-neutral" onClick={handleStopOrRetry}>
  ...
  return (
    <div className="flex-1 overflow-hidden relative">
      ...
      <div className={classNames('absolute bottom-0 left-0 w-full bg-vert-light-gradient dark:bg-vert-dark-gradient z-20', { 'md:px-4 max-md:pb-4': !checkFlag })}>
        {!checkFlag ? assistant && (
          <div className="flex flex-row pt-2 mx-auto max-w-4xl md:pb-6">
            <div className="relative flex h-full flex-1 flex-col max-md:px-2">
              ...
              <AutoTextArea loading={loading || assistantLoading} onFetchAnswer={(value) => setQuestion(value.text)} />
            </div>
          </div>
        ) : (
